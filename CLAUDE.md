...
- **Excepción documentada (decisión Nico, 2026-07-22):** las **contraseñas de PDF de extractos** (`accounts.pdf_password`, `institutions.pdf_password`) **SÍ se persisten**, pero **cifradas** con AES-256-GCM (`lib/crypto/secret-box.ts`, clave maestra en la env var `PDF_PASSWORD_ENC_KEY`, payload versionado `v1:<iv>:<tag>:<ct>`). Son necesarias porque la app debe abrir el PDF al parsear, así que el cifrado es reversible y NO puede reemplazarse por un hash. Reglas: **nunca en claro en DB** (constraint `CHECK … LIKE 'v1:%'`, migración `0019`), **nunca al cliente** (el form es write-only; la UI solo recibe un booleano "hay contraseña guardada"), **nunca en logs** (ni el valor cifrado ni el plano). Sin la clave, guardar **falla explícito**; jamás cae de vuelta a texto plano. Rotar `PDF_PASSWORD_ENC_KEY` invalida lo cifrado → hay que recargar las contraseñas a mano.
